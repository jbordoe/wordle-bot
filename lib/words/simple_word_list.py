...
class SimpleWordList(WordSetInterface):
    ...
    def find_words(
        self, placed_letters=None, contains=set(), filter=set(), excludes=None
    ):
        ...
        for w in self.words:
            invalid = False
            if w["set"].intersection(filter):
                continue
            for i, w_i in enumerate(w["str"]):
                if placed[i] and placed[i] != w_i:
                    invalid = True
                if w_i in excludes[i]:
                    invalid = True
                if invalid:
                    break
            if invalid or not w["set"].issuperset(contains):
                continue
