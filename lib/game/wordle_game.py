...
class WordleGame(GameInterface):
    ...
    def _begin_game(self):
        ...
        try:
            close_modal = WebDriverWait(self.browser.driver, 10).until(
                EC.presence_of_element_located(
                    (By.XPATH, '//button[contains(@class, "Modal-module_closeIcon__")]')
                )
            )
            close_modal.click()
        except (TimeoutException, NoSuchElementException) as e:
    ...
    def _press_key(self, letter):
        ...
        key_el = keyboard.find_element(
            "css selector", f'button[data-key="{letter.lower()}"]'
        )
    ...
    def _game_lost(self):
        try:
            self.browser.find_element('div#game-toaster[duration="infinity"]')
            return True
    ...
    def _get_last_row(self):
        ...
        return [
            r
            for r in rows
            if all(t.get_attribute("data-state") in valid_states for t in r)
        ][-1]

    def _find_els_by_class_substr(self, substr, tag="*"):
        return self.browser.driver.find_elements(
            by=By.XPATH, value=f'//{tag}[contains(@class, "{substr}")]'
        )
